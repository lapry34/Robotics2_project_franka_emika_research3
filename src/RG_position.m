...
qA_idx = [1,2,4]; % indices of joints in A (nonsingular)
qB_idx = [3, 5, 6, 7]; % indices of joints in B (N-M = 4)
...
while t < t_fin % run for a fixed time
    ...
    % [!] RG step
    dq = reduced_grad_step(q, dp_nom, qA_idx, qB_idx, p_nom, 10); % compute joint velocity using reduced gradient step
    disp(['dq = [', num2str(dq'), ']']);
