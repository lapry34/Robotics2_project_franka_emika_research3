...
T = 5; % total time for the trajectory [s]
...
p_x_t = C(1);
p_y_t = C(2) + R * cos(2*pi*freq  * t_sym + gamma);
p_z_t = C(3) + R * sin(2*pi*freq * t_sym + gamma);
...
scatter3(p_sing(1), p_sing(2), p_sing(3), 50, 'filled', 'MarkerFaceColor', 'k'); % Singularity point in black
scatter3(p_in(1), p_in(2), p_in(3), 50, 'filled', 'MarkerFaceColor', 'g'); % Start point in green
scatter3(p_fin(1), p_fin(2), p_fin(3), 50, 'filled', 'MarkerFaceColor', 'b'); % End point in blue
legend('Actual End-Effector Path', 'Nominal Trajectory', 'Singularity Point', 'Start Point', 'End Point');
