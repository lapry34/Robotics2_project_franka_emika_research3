...
while t < t_fin % run for a fixed time
    ...
    J_dot = get_J_dot(q, q_dot, true); % compute the time derivative of the Jacobian
    ...
    r_dot = J * q_dot;
    r_ddot = J_dot * q_dot + J * q_ddot; % compute end-effector acceleration
    ...
    if print_info == true
        disp(['ddq (after clamping) = [', num2str(ddq'), ']']);
    end
    ...
    if print_info == true
        disp(['dq (after clamping) = [', num2str(dq'), ']']);
    end
...
%% Moving the figures
save_imgs_path = "figures\RG_orientation_acceleration\";
